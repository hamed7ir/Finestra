...
{
    /// <summary>
    /// FRDP-SSH-AUTH — view / remove the TOFU <see cref="KnownHosts"/> entries. Removing one makes the next connect
    /// to that host prompt fresh, so a legitimately-rekeyed server is recoverable rather than a permanent
    /// "host key changed" dead end. Opened from <see cref="AppSettingsForm"/>.
    /// </summary>
    public sealed class KnownHostsForm : ThemedDialog
    {
        private const int W = 560;

        public KnownHostsForm() : base("Known SSH hosts", W, 520)
        {
            Rebuild();
        ...
        {
            KnownHosts.Reload();
            var items = KnownHosts.Instance.Items;
            int w = W - 40;
            ...
            if (items.Count == 0)
            {
                PopulateBody(new InfoBlock(w,
                    "No SSH host keys have been remembered yet.\n\nWhen you accept a server's key on first connect, " +
                    "it's stored here so a later change can be flagged.", loud: false));
                return;
            }

            var rows = new List<Control>
            {
                new InfoBlock(w, "These are the SSH servers whose host keys you've trusted. Remove one to be prompted " +
                                 "fresh next time you connect — use this if a server was legitimately rebuilt or rekeyed.", loud: false)
            };
            foreach (var e in items.OrderBy(x => x.Host).ThenBy(x => x.Port))
            {
                var row = new KnownHostRow(e, w);
                row.RemoveClicked += ent => { KnownHosts.Instance.Remove(ent.Host, ent.Port); Rebuild(); };
                rows.Add(row);
            }
    ...
    /// <summary>One known-host line: host:port + key type + fingerprint + first-seen, with a Remove button.</summary>
    internal sealed class KnownHostRow : Control
    {
        private readonly KnownHostEntry _e;
        private readonly RoundedButton _remove;
        public event Action<KnownHostEntry> RemoveClicked;

        public KnownHostRow(KnownHostEntry e, int width)
        {
            _e = e;
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            using (var tf = FontHelper.Ui(11f, FontStyle.Bold))
                TextRenderer.DrawText(g, _e.Host + ":" + _e.Port + "    " + _e.KeyType, tf,
